...
void processEventListPriority(){
	...
	while (!event_list.empty()){
		...
		eventType type = e.type;
		int id = e.process_id;
		printf("PID = %d \tTIME = %d\tTYPE = %s \tREMAINING=%d \n",e.process_id,(int)e.time,e.printType().c_str(), process_list[id-1].currentCPUTime);
		...
		if (type == CPU_START) {
				...
				if (c.p.start_priority < process_list[id-1].start_priority){
					...
					if (time_slice >= process_list[id-1].currentCPUTime){
						// if (time_slice > process_list[id-1].currentCPUTime){
							...
							// printf("New HIGH Priority = %d\n",process_list[id-1].start_priority );
						// }
						Event k(id, CPU_END, present_time + process_list[id-1].currentCPUTime);
						event_list.push(k);
						process_list[id-1].currentCPUTime = 0;
			...
			c.busy = false;
			if (process_list[id-1].start_time+mymap[process_list[id-1].start_priority] == present_time){
				process_list[id-1].start_priority = getLowerPriority(process_list[id-1].start_priority );
				printf("New LOW Priority = %d\n",process_list[id-1].start_priority );
